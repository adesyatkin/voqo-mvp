...
# Настройка логгера для унифицированного модуля
def setup_logging():
...
class AudioProcessorUnified:
    ...
    def __init__(self):
        ...
        self.whisper_chunk_files_dir = Path("D:/VOQO/workers/whisperchunk_files")
        self.temp_dir = None
        ...
        self.whisper_chunk_files_dir.mkdir(parents=True, exist_ok=True)
        ...
        logger.info(f"Выходная папка для whisper-чанков: {self.whisper_chunk_files_dir}")
    ...
    def setup_configuration(self):
        ...
        # Параметры для whisper-чанков
        self.whisper_chunk_length = 25  # Длина чанка для транскрибации
        self.whisper_chunk_overlap = 5  # Перекрытие между чанками
    ...
    def create_whisper_chunks(self, audio_file: Path, base_name: str) -> Dict:
        """Создает чанки для транскрибации с перекрытием в отдельную папку"""
        whisper_result = {
            'chunks_created': 0,
            'errors': [],
            'warnings': []
        ...
        try:
            logger.info(f"🎵 Создание whisper-чанков для транскрибации: {audio_file.name}")
            
            # Проверяем существование файла
            if not audio_file.exists():
                error_msg = f"Файл не найден: {audio_file}"
                whisper_result['errors'].append(error_msg)
            ...
            total_duration = audio_info['duration']
            logger.info(f"📊 Длительность файла для whisper: {total_duration:.2f} сек")
            
            # Если файл слишком короткий, создаем один чанк
            if total_duration <= self.whisper_chunk_length:
                segments = [{
                    'start': 0,
                    'end': total_duration,
                    'duration': total_duration
                ...
                # Создаем сегменты по 25 секунд с перекрытием 5 секунд
                segments = []
                start = 0
                chunk_num = 1
                
                while start < total_duration:
                    end = min(start + self.whisper_chunk_length, total_duration)
                    segments.append({
                        'start': start,
                        'end': end,
                        'duration': end - start
                    ...
                    start += (self.whisper_chunk_length - self.whisper_chunk_overlap)
                    chunk_num += 1
            
            logger.info(f"📊 Нарезано {len(segments)} сегментов для whisper")
            ...
            # Создаем чанки в отдельной папке
            for idx, segment in enumerate(segments, 1):
                ...
                # Создаем имя файла с единым форматом: имя_whisperchank_начало-конец.wav
                output_filename = f"{base_name_clean}_whisperchank_{start_str}-{end_str}.wav"
                output_path = self.whisper_chunk_files_dir / output_filename
                ...
                if output_path.exists():
                    ...
                    suffix = 1
                    while output_path.exists():
                        output_filename = f"{base_name_clean}_whisperchank_{start_str}-{end_str}_{suffix}.wav"
                        output_path = self.whisper_chunk_files_dir / output_filename
                        suffix += 1
                ...
                try:
                    ...
                    if output_path.exists():
                        ...
                        # Показываем реальное время в логах
                        hours_start = start_time // 3600
                        minutes_start = (start_time % 3600) // 60
                        seconds_start = start_time % 60
                        ...
                        logger.info(f"🎵 Whisper-чанк {idx}: {output_filename} ({duration:.2f} сек, {chunk_size_kb:.1f} КБ)")
                        logger.info(f"   [время: {time_format_start}-{time_format_end}]")
                        logger.info(f"   📁 Сохранен в: {self.whisper_chunk_files_dir}")
                        
                        whisper_result['chunks_created'] += 1
                    else:
                        error_msg = f"Не удалось создать whisper-чанк: {output_filename}"
                        whisper_result['errors'].append(error_msg)
                        logger.error(f"❌ {error_msg}")
                        
                except Exception as e:
                    error_msg = f"Ошибка создания whisper-чанка {idx}: {e}"
                    whisper_result['errors'].append(error_msg)
                    logger.error(f"❌ {error_msg}")
            
            if whisper_result['chunks_created'] > 0:
                logger.info(f"✅ Создано whisper-чанков: {whisper_result['chunks_created']}")
                logger.info(f"📁 Папка с whisper-чанками: {self.whisper_chunk_files_dir}")
            else:
                logger.warning(f"⚠️  Не создано ни одного whisper-чанка")
                
        except Exception as e:
            error_msg = f"Критическая ошибка в create_whisper_chunks: {e}"
            whisper_result['errors'].append(error_msg)
            logger.error(f"❌ {error_msg}")
            import traceback
            logger.error(traceback.format_exc())
    ...
    def create_mono_for_whisper(self, audio_file: Path, base_name: str) -> Optional[Path]:
        """Создает моно-версию файла для транскрибации"""
        try:
            whisper_mono_file = self.temp_dir / f"{base_name}_whisper_mono.wav"
            
            # Проверяем, является ли файл уже моно
            audio_info = self.get_audio_info(audio_file)
            if audio_info and audio_info['channels'] == 1:
                # Файл уже моно, просто копируем
                shutil.copy2(audio_file, whisper_mono_file)
                logger.info(f"✅ Файл уже моно, создана копия для whisper")
            else:
                # Конвертируем в моно
                logger.info(f"🎵 Конвертация в моно для whisper-чанков...")
                
                (
                    ffmpeg
                    .input(str(audio_file))
                    .output(
                        str(whisper_mono_file),
                        acodec='pcm_s16le',
                ...
                if whisper_mono_file.exists():
                    file_size_mb = whisper_mono_file.stat().st_size / (1024 * 1024)
                    logger.info(f"✅ Моно-версия для whisper создана: {file_size_mb:.2f} МБ")
                else:
                    logger.error(f"❌ Не удалось создать моно-версию для whisper")
                    return None
            
            return whisper_mono_file
            
        except Exception as e:
            logger.error(f"❌ Ошибка создания моно-версии для whisper: {e}")
            return None
    ...
    def renew_pyannote_api_key(self) -> bool:
        """Пересоздание API-ключа pyannote"""
        try:
            logger.warning("🔄 Попытка пересоздания API-ключа...")
            
            renewal_script = Path("D:/VOQO/auto_pyannote_daily_registration.py")
            
            if not renewal_script.exists():
                logger.error(f"❌ Скрипт пересоздания не найден: {renewal_script}")
                return False
            
            logger.info(f"🔄 Запуск скрипт пересоздания ключа...")
            start_time = time.time()
            
            result = subprocess.run(
                [sys.executable, str(renewal_script)],
                capture_output=True,
                text=True,
                timeout=120
            )
            ...
            logger.info(f"⏱️  Скрипт выполнен за {execution_time:.1f} секунд")
            
            if result.returncode == 0:
                logger.info("✅ API-ключ успешно пересоздан")
                
                # Ожидание обновления конфигурации
                logger.info("⏳ Ожидание обновления конфигурации (5 секунд)...")
                time.sleep(5)
            ...
            logger.error("❌ Таймаут при пересоздании API-ключа")
            return False
        except Exception as e:
            logger.error(f"❌ Ошибка при пересоздании API-ключа: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False
    
    # ================== НОВЫЕ МЕТОДЫ ДЛЯ РАБОТЫ С БОЛЬШИМИ ФАЙЛАМИ ==================
    
    def _split_audio_into_chunks(self, audio_file: Path) -> List[Dict]:
        """
        Разбивает аудиофайл на перекрывающиеся чанки.
        Возвращает список словарей: {'path': Path, 'start': float, 'end': float}
        """
        ...
        for i in range(num_chunks):
            ...
            # Если чанк слишком короткий (меньше 1 сек), пропускаем (обычно последний может быть коротким)
            if end - start < 1.0:
            ...
            try:
                (
                    ...
                    .output(
                        str(chunk_filename),
                        acodec='pcm_s16le',
                        ac=1,          # диаризация всегда на моно
                        ar=self.target_sample_rate
                    )
                    .overwrite_output()
                    .run(quiet=True, capture_stderr=True)
    ...
    def _diarize_single_file(self, file_path: Path, file_start_time: float) -> Optional[List[Dict]]:
        """
        Отправляет один аудиофайл (чанк) через Telegram в pyannote, получает сегменты
        и преобразует их времена к абсолютным (прибавляя file_start_time).
        Возвращает список сегментов с абсолютными временами.
        """
        ...
        for attempt in range(1, self.max_diarization_retries + 1):
            try:
                ...
                # Преобразуем времена к абсолютным
                for seg in segments:
    ...
    def _match_speakers_across_chunks(self, chunks_data: List[Dict]) -> List[Dict]:
        ...
        chunks_data: список словарей с ключами:
            - 'segments': список сегментов чанка (с абсолютными временами и локальными метками)
            - 'start': время начала чанка
            - 'end': время конца чанка
        Возвращает единый список сегментов с глобальными номерами спикеров.
        Глобальный номер спикера сохраняется в поле 'speaker_global' (int).
        """
        if not chunks_data:
            return []
        
        # Сортируем чанки по времени начала
        chunks_data.sort(key=lambda x: x['start'])
        
        # Глобальный маппинг: {локальная_метка: глобальный_номер}
        global_map = {}
        next_global = 1
        
        # Список для хранения всех сегментов с глобальными номерами
        all_segments = []
        ...
        for seg in first_chunk['segments']:
            ...
            if local_spk not in global_map:
                ...
                next_global += 1
            # Добавляем копию сегмента с глобальным номером
            all_segments.append({
                'start': seg['start'],
                'end': seg['end'],
        ...
        # Обрабатываем последующие чанки
        for i in range(1, len(chunks_data)):
            curr_chunk = chunks_data[i]
            prev_chunk = chunks_data[i-1]
            
            # Интервал перекрытия: [curr_chunk['start'], prev_chunk['end']]
            overlap_start = curr_chunk['start']
            overlap_end = prev_chunk['end']
            # Если перекрытие отрицательное (разрыв), то сопоставление не по перекрытию, а по ближайшему времени?
            # Но мы создавали чанки с перекрытием, так что overlap_end > overlap_start.
            if overlap_end <= overlap_start:
                logger.warning(f"  Чанки {i-1} и {i} не перекрываются, сопоставление будет приблизительным")
                # В этом случае можно попробовать сопоставить по близости во времени,
                # но для простоты назначим новые номера всем новым спикерам.
                # Однако лучше сделать так: если нет перекрытия, то все спикеры текущего чанка считаем новыми.
            ...
            # Получаем сегменты в перекрытии из предыдущего чанка (уже с глобальными номерами)
            prev_overlap = [s for s in all_segments if s['end'] > overlap_start and s['start'] < overlap_end]
            # Получаем сегменты в перекрытии из текущего чанка (с локальными метками)
            curr_overlap = [s for s in curr_chunk['segments'] if s['end'] > overlap_start and s['start'] < overlap_end]
            
            # Строим матрицу пересечений: для каждого локального спикера из curr_overlap и глобального из prev_overlap
            # Словарь: {локальная_метка: {глобальный_номер: суммарная_длительность_пересечений}}
            import numpy as np
            overlap_matrix = {}
            # Инициализируем
            ...
            # Для каждой пары сегментов считаем пересечение
            for seg_curr in curr_overlap:
                local_spk = seg_curr['speaker']
            ...
            used_globals = set()
            # Сначала для каждого локального спикера ищем глобального с максимальным пересечением
            for local_spk, matches in overlap_matrix.items():
                if not matches:
                    continue
                # Находим глобального с максимальным overlap
                ...
                # Если порог не достигнут или глобальный уже занят, пока оставляем не сопоставленным
                # (будет обработано ниже)
            
            # Теперь всем локальным спикерам, которые не получили сопоставление, назначаем новые глобальные номера
            for seg in curr_chunk['segments']:
                local_spk = seg['speaker']
                if local_spk not in global_map:
                    global_map[local_spk] = next_global
                    next_global += 1
                # Добавляем сегмент с глобальным номером
                all_segments.append({
                    'start': seg['start'],
                    'end': seg['end'],
                    'speaker_global': global_map[local_spk],
        ...
        # Сортируем все сегменты по времени
        all_segments.sort(key=lambda x: x['start'])
        return all_segments
    
    def _obtain_diarization_segments(self, audio_file: Path) -> Optional[List[Dict]]:
        """
        Основной метод для получения сегментов диаризации с учётом возможной разбивки.
        Возвращает список сегментов с абсолютными временами и глобальными номерами спикеров
        в поле 'speaker_global' (int).
        """
        # Проверяем размер файла
        file_size_mb = audio_file.stat().st_size / (1024 * 1024)
        logger.info(f"Размер файла для диаризации: {file_size_mb:.1f} МБ")
        
        # Если файл маленький, обрабатываем целиком
        if file_size_mb <= self.telegram_file_size_limit_mb:
            ...
            if not segments:
                return None
            # Преобразуем в формат с глобальными номерами
            global_map = {}
            next_global = 1
            result = []
            for seg in segments:
        ...
        # Файл большой – разбиваем на чанки
        ...
        # Для каждого чанка получаем сегменты
        chunks_data = []
        for chunk in chunks:
            ...
            if segs is None:
                ...
                # Можно попробовать продолжить с остальными, но лучше вернуть None
                return None
            chunks_data.append({
                'segments': segs,
                'start': chunk['start'],
        ...
        # Сопоставляем спикеров между чанками
    ...
    def process_with_diarization(self, audio_file: Path, base_name: str, 
        ...
        Возвращает список созданных чанков датасета.
        """
        ...
        try:
            ...
            # Определяем файл для диаризации (как и раньше)
            # Здесь audio_file – это уже подготовленный файл (первый канал или моно)
            file_for_diarization = audio_file
            
            # Получаем сегменты с глобальными номерами спикеров
            ...
            # Группируем сегменты по глобальным номерам спикеров
            from collections import defaultdict
            speaker_segments = defaultdict(list)
            for seg in segments_with_global:
                speaker_num = seg['speaker_global']
                # Для совместимости с create_unified_chunks создаём поле 'speaker' вида 'SPEAKER_XX'
                seg_copy = seg.copy()
                seg_copy['speaker'] = f"SPEAKER_{speaker_num-1:02d}"  # чтобы нумерация с 0
            ...
            for speaker, seg_list in speaker_segments.items():
                ...
                logger.info(f"  👤 {speaker} (глобальный {speaker_num}): {len(speaker_chunks)} чанков")
    ...
    def _handle_retry_delay(self, attempt: int):
        ...
        if attempt == 1:
            # Первая ошибка: пересоздаем ключ и ждем
    ...
    def get_input_files(self) -> List[Path]:
        ...
        # Убираем дубликаты по имени (разный регистр)
        unique_files = []
        seen_names = set()
    ...
    def process_single_file(self, input_file: Path) -> Dict:
        """Полная обработка одного файла с параллельным созданием whisper-чанков"""
        file_result = {
        ...
        try:
            ...
            # Шаг 3: Параллельно запускаем создание whisper-чанков
            logger.info(f"🎵 ЗАПУСК ПАРАЛЛЕЛЬНОГО СОЗДАНИЯ WHISPER-ЧАНКОВ...")
            
            def whisper_processing():
                """Функция для запуска в отдельном потоке"""
                try:
                    # Создаем моно-версию для whisper
                    whisper_mono_file = self.create_mono_for_whisper(converted_file, input_file.stem)
                    if whisper_mono_file and whisper_mono_file.exists():
                        # Создаем whisper-чанки
                        result = self.create_whisper_chunks(whisper_mono_file, input_file.stem)
                        whisper_result.update(result)
                    else:
                        whisper_result.update({
                            'chunks_created': 0,
                            'errors': ['Не удалось создать моно-версию для whisper'],
                            'warnings': []
                        })
                except Exception as e:
                    whisper_result.update({
                        'chunks_created': 0,
                        'errors': [f'Критическая ошибка в whisper-потоке: {str(e)}'],
                        'warnings': []
                    })
                    logger.error(f"❌ Ошибка в whisper-потоке: {e}")
            
            # Запускаем поток для whisper-чанков
            whisper_thread = threading.Thread(target=whisper_processing)
            whisper_thread.daemon = True
            whisper_thread.start()
            logger.info(f"✅ Поток для whisper-чанков запущен")
            
            # Продолжаем основную обработку параллельно
            ...
            # Шаг 4: Разделение на каналы и анализ
            ...
            if not channel_files:
                ...
                # Ждем завершения whisper-потока
                if whisper_thread and whisper_thread.is_alive():
            ...
            # Шаг 6: Обработка по выбранной стратегии
            if channel_analysis['recommendation'] == 'channel_based':
                ...
                # Определяем, какой файл использовать для диаризации
            ...
            # Шаг 7: Ждем завершения whisper-потока
            logger.info(f"⏳ Ожидание завершения whisper-потока...")
            if whisper_thread and whisper_thread.is_alive():
                whisper_thread.join(timeout=300)  # Таймаут 5 минут
            
            # Обновляем результаты whisper-обработки
            if whisper_result:
                file_result['whisper_chunks_created'] = whisper_result.get('chunks_created', 0)
                file_result['whisper_errors'] = whisper_result.get('errors', [])
                whisper_warnings = whisper_result.get('warnings', [])
                if whisper_warnings:
                    file_result['warnings'].extend([f"Whisper: {w}" for w in whisper_warnings])
            ...
            logger.info(f"  • Whisper-чанков создано: {file_result['whisper_chunks_created']}")
            ...
            # Пытаемся завершить whisper-поток в случае ошибки
            if whisper_thread and whisper_thread.is_alive():
        ...
        try:
            ...
            for file in input_files:
                try:
                    ...
                    if (result['chunks_created'] > 0 or result['whisper_chunks_created'] > 0) and not result['errors']:
                        stats['successful'] += 1
                        stats['total_chunks'] += result['chunks_created']
                        stats['total_whisper_chunks'] += result['whisper_chunks_created']
                        stats['total_channels'] += result['channels_found']
                        
                        if result.get('used_fallback', False):
                            stats['files_with_fallback'] += 1
                            logger.warning(f"⚠️  {file.name}: {result['chunks_created']} чанков + {result['whisper_chunks_created']} whisper (FALLBACK)")
                        else:
                            logger.info(f"✅ {file.name}: {result['chunks_created']} чанков + {result['whisper_chunks_created']} whisper")
                    else:
                        stats['failed'] += 1
                        stats['errors'].extend(result['errors'])
                        stats['errors'].extend([f"Whisper: {e}" for e in result.get('whisper_errors', [])])
                        stats['warnings'].extend(result.get('warnings', []))
                        if result['errors'] or result.get('whisper_errors'):
                            error_count = len(result['errors']) + len(result.get('whisper_errors', []))
            ...
            logger.info(f"Всего whisper-чанков: {stats['total_whisper_chunks']}")
            ...
            logger.info(f"📁 Whisper-чанки сохранены в: {self.whisper_chunk_files_dir}")
            logger.info("="*70)
