...
from adapters import ParakeetAdapter

INPUT_FOLDER = "chunk_files"
OUTPUT_FOLDER = "transcription_parakeet"

def setup_logging():
    log_dir = Path(__file__).parent / "logs"
    log_dir.mkdir(exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = log_dir / f"transcription_parakeet_{timestamp}.log"
    logging.basicConfig(
        level=logging.INFO,
...
def main():
    ...
    logger.info("🎧 PARAKEET-1_1b-RNNT-ASR - ТРАНСКРИПЦИЯ")
