...
def check_required_folders():
    required_folders = ['chunk_files', 'logs', 'transcription_whisper']
...
def main():
    ...
    try:
        ...
        output_folder = "transcription_whisper"
        
        if not os.path.exists(input_folder):
            logger.error(f"❌ Папка '{input_folder}' не найдена")
            logger.info(f"📌 Создайте папка '{input_folder}' и поместите туда WAV файлы для обработки")
            time.sleep(1)
            return
