...
def check_required_folders():
    required_folders = [
        ...
        'logs',
        'D:\\VOQO\\workers\\context_whisper'
    ]

    for folder in required_folders:
        if not os.path.exists(folder):
            os.makedirs(folder)
            print(f"[CONTEXT-WHISPER] 📁 Создана папка: {folder}")
...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f"context_whisper_{timestamp}.log")
...
def save_individual_result(result, output_folder, logger):
    try:
        ...
        # Заменяем "_context_" на "_context_whisper_"
        if '_context_' in base_name:
            new_base = base_name.replace('_context_', '_context_whisper_')
        else:
            new_base = base_name
...
def main():
    ...
    try:
        ...
        output_folder = "D:\\VOQO\\workers\\context_whisper"

        if not os.path.exists(input_folder):
            logger.error(f"❌ Папка '{input_folder}' не найдена")
            logger.info(f"📌 Убедитесь, что папка context_files существует")
            time.sleep(1)
            return
        ...
        logger.info("🎧 CONTEXT WHISPER (Whisper-large-v3) - ТРАНСКРИПЦИЯ КОНТЕКСТНЫХ ЧАНКОВ")
        logger.info("=" * 50)
